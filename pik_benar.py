...
# Load awal
soal_data = load_json_file(SOAL_FILE, DEFAULT_SOAL)
skor_data = load_json_file(SKOR_FILE, [])


# -------------------------------
# STREAMLIT UI
# -------------------------------
st.title("📘 Aplikasi Quiz Sederhana (Streamlit Improved Version)")

menu = st.sidebar.radio(
    "Menu",
    ["Mulai Quiz", "Tambah Soal", "Lihat Riwayat Nilai", "Lihat Grafik", "Keluar"]
)
...
if menu == "Mulai Quiz":
    ...
    if not soal_data:
        st.warning("Belum ada soal. Tambah soal terlebih dahulu.")
        st.stop()

    if "nama" not in st.session_state:
        st.session_state.nama = ""

    st.session_state.nama = st.text_input("Masukkan Nama Peserta:")

    if st.session_state.nama == "":
        st.info("Silakan masukkan nama sebelum mulai quiz.")
        st.stop()

    if "soal_list" not in st.session_state:
        st.session_state.soal_list = soal_data.copy()
        random.shuffle(st.session_state.soal_list)
        st.session_state.index = 0
        st.session_state.jawaban_user = []

    total_soal = len(st.session_state.soal_list)
    current = st.session_state.index
    soal = st.session_state.soal_list[current]

    st.subheader(f"Soal {current + 1} dari {total_soal}")
    st.progress((current + 1) / total_soal)

    st.write("### Pertanyaan:")
    st.write(soal["pertanyaan"])

    pilihan = soal["pilihan"]
    jawaban = st.radio("Pilih Jawaban:", pilihan, key=f"pilihan_{current}")

    if st.button("Next Soal ➜"):
        st.session_state.jawaban_user.append(jawaban)
        st.session_state.index += 1

        if st.session_state.index >= total_soal:
            benar = 0
            for i, s in enumerate(st.session_state.soal_list):
                if st.session_state.jawaban_user[i] == s["pilihan"][s["jawaban_index"]]:
                    benar += 1
            ...
            st.success(f"Nilai Akhir: {skor}")
            st.info(f"Jawaban Benar: {benar} / {total_soal}")

            for key in ["soal_list", "jawaban_user", "index"]:
...
# ==========================================
    ...
    pertanyaan = st.text_input("Masukkan pertanyaan:")
    pilihan_input = st.text_area("Masukkan pilihan jawaban (pisahkan dengan enter):")

    pilihan = [p.strip() for p in pilihan_input.split("\n") if p.strip() != ""]

    jawaban_benar = st.number_input("Nomor jawaban benar (mulai dari 0)", min_value=0, step=1)

    if st.button("Simpan Soal"):
        if pertanyaan and len(pilihan) >= 2:
            soal_baru = {
                "pertanyaan": pertanyaan,
                "pilihan": pilihan,
                "jawaban_index": int(jawaban_benar)
            }
            soal_data.append(soal_baru)
            save_json_file(SOAL_FILE, soal_data)
            st.success("Soal berhasil ditambahkan!")
        else:
            st.error("Pertanyaan dan minimal 2 pilihan harus diisi.")


# ==========================================
# 3. RIWAYAT NILAI
# ==========================================
elif menu == "Lihat Riwayat Nilai":
    st.header("📊 Riwayat Nilai Siswa")

    if not skor_data:
        st.warning("Belum ada riwayat nilai.")
    else:
        for i, e in enumerate(skor_data, start=1):
            st.write(
                f"**{i}. {e['timestamp']} — {e['nama']} | Nilai: {e['skor']} | Benar: {e['benar']}/{e['total']}**"
            )
...
# 4. GRAFIK NILAI
# ==========================================
elif menu == "Lihat Grafik":
    st.header("📈 Grafik Perkembangan Nilai")

    if not skor_data:
        st.warning("Belum ada data.")
    else:
        timestamps = [e["timestamp"].replace("T", " ") for e in skor_data]
        skor_list = [e["skor"] for e in skor_data]

        plt.figure(figsize=(8, 4))
        plt.plot(timestamps, skor_list, marker='o')
        plt.xticks(rotation=45)
        plt.xlabel("Waktu")
        plt.ylabel("Nilai")
        plt.title("Perkembangan Nilai Quiz")
        st.pyplot(plt)
...
# ==========================================
    ...
    st.header("👋 Terima kasih sudah menggunakan aplikasi!")
    st.stop()